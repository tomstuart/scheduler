...
describe Calendar do
  ...
  describe '#periods' do
    let(:start_time) { Time.local(2013, 12, 2, 0, 0) }
    let(:end_time) { Time.local(2013, 12, 8, 23, 59, 59) }
    ...
    before(:each) do
      ...
      Period.stub(:new).with(Time.local(2013, 12, 2, 9, 0), Time.local(2013, 12, 2, 17, 0)).and_return(monday_period)
      Period.stub(:new).with(Time.local(2013, 12, 3, 9, 0), Time.local(2013, 12, 3, 17, 0)).and_return(tuesday_period)
      Period.stub(:new).with(Time.local(2013, 12, 4, 9, 0), Time.local(2013, 12, 4, 17, 0)).and_return(wednesday_period)
      Period.stub(:new).with(Time.local(2013, 12, 5, 9, 0), Time.local(2013, 12, 5, 17, 0)).and_return(thursday_period)
      Period.stub(:new).with(Time.local(2013, 12, 6, 9, 0), Time.local(2013, 12, 6, 17, 0)).and_return(friday_period)

      stub_const('PeriodGroup', double('fake PeriodGroup class', new: periods))
    end

    it 'should create the Monday period' do
      Period.should_receive(:new).with(Time.local(2013, 12, 2, 9, 0), Time.local(2013, 12, 2, 17, 0))
      calendar.periods
    end

    it 'should create the Tuesday period' do
      Period.should_receive(:new).with(Time.local(2013, 12, 3, 9, 0), Time.local(2013, 12, 3, 17, 0))
      calendar.periods
    end

    it 'should create the Wednesday period' do
      Period.should_receive(:new).with(Time.local(2013, 12, 4, 9, 0), Time.local(2013, 12, 4, 17, 0))
      calendar.periods
    end

    it 'should create the Thursday period' do
      Period.should_receive(:new).with(Time.local(2013, 12, 5, 9, 0), Time.local(2013, 12, 5, 17, 0))
      calendar.periods
    end

    it 'should create the Friday period' do
      Period.should_receive(:new).with(Time.local(2013, 12, 6, 9, 0), Time.local(2013, 12, 6, 17, 0))
      calendar.periods
    end
